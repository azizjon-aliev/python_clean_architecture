...
class AbstractRepository[T: Model, TKey]:
    ...
    def __init__(self):
        ...
        """
        if not hasattr(self, 'model') or not isinstance(self.model, type) or not issubclass(self.model, Model):
    ...
    def delete(self, object_id: TKey) -> None:
        ...
        if not instance:
            raise EntityDoesNotExist(f"{self.model.__name__} with id {object_id} not found")
        instance.delete()
